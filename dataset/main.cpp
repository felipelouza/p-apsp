...
//			cout<<"p = "<<p<<", "<<Min_lcp[p]<<"\t"<<Lglobal[i]->value<<endl;
	
			while(Lglobal[i]->value > Min_lcp[p]){
...
//			prepend(Lg, Ll, next, i, sentinel);
			prepend(Lglobal, Llocal[p], Next[p], i, sentinel);
				...
				if(Lglobal[i]->value)
            		#endif
				if(!result[prefix][i])
					result[prefix][i] = Lglobal[i]->value;
