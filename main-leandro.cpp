...
int main(int argc, char *argv[]){
	...
	uint_t threshold;
	sscanf(argv[3], "%" PRIdN "", &threshold);
	printf("L: %" PRIdN "\n", threshold);

	uint_t output;
	sscanf(argv[4], "%" PRIdN "", &output);
...
//	#endif
    ...
    #pragma omp for schedule(dynamic) reduction(+:overlaps,removes)
...
//	printf("### [%d] = %d ###\n", omp_get_thread_num(), t);
	...
	#pragma omp for reduction(+:contained) //firstprivate(threshold,n,k)
	for(uint_t p = 0; p < k; p++){
